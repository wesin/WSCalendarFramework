...
class WSCalendarItem: UICollectionViewCell,UICollectionViewDataSource,UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    weak var delegate:WSCalendarItemDelegate?
...
//            println(curDay.formattedDateWithFormat("yyyy-MM-dd"))
            ...
            //选中的日期则设为选中
            if curSelectDate != nil {
                if curDay.year() == curSelectDate!.year() && curDay.month() == curSelectDate!.month() && curDay.day() == curSelectDate!.day() {
                    cell?.setSelect(true)
